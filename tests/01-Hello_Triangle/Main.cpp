...
int main()
{
	...
	const int checkerW = 2, checkerH = 2;
	int checker_offset = 0.1;
	glm::vec4 c0 = { 0, 1, 0, 1 }, c1 = {0, 0, 1, 1};
	

	Vertex verts[(checkerW+1) * (checkerH+1)];

	for (int ii = 0; ii < (checkerH + 2); ++ii)
	{
		for (int jj = 0; jj < (checkerW + 2); ++jj)
		{
			verts[ii*(checkerW+1) + jj].position = { 0.1f * jj, 0.1f*ii, 0, 1 };
			verts[ii*(checkerW + 1) + jj].color = ii % 2 == 0 ? c0 : c1;
		}
	}
